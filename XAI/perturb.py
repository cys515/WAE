...
class Perturbation(AbstractPerturbation):
    ...
    def get_sample(cls, x, segm, r=None, p_off=0.5):
        ...
        z_prime = cls._get_on_off_segments(segm, p_off) #vector [0 or 1], len=window_num
        mask = cls._get_segment_mask(segm, z_prime) #[[0,0,...,0] or [1,1,...,1]], shape=n_steps*features
    ...
    def get_samples(cls, x, segm, replace_method='zeros', p_off=0.5, n_samples=10):
        ...
        """
        fn = getattr(repl, replace_method) #repl.replace_method
        r = fn(x, segm) #masks

        for _ in range(n_samples): #create n samples with window_mask and similarity
